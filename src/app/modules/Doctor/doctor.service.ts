...
import { IDoctorFilterRequest, IDoctorUpdate } from "./doctor.intreface";
...
const updateIntoDB = async (id: string, payload: IDoctorUpdate) => {
  ...
  await prisma.$transaction(async (transactionClient) => {
    ...
    if (specialties && specialties.length > 0) {
      ...
      // console.log({deleteSpecialtiesId});
      for (const specialty of deleteSpecialtiesId) {
        const d = await transactionClient.doctorSpecialties.deleteMany({
          where: {
